    ...
    const emxArray_real_T *costabinv, const emxArray_real_T *sintabinv)
{
  ...
  if (!tst) {
    ...
    temp_re = wwc_data[nd2].re;
    temp_im = wwc_data[nd2].im;
    if ((unsigned short)n2 - 1 < 0) {
      nd2 = 0;
    } else {
      nd2 = (unsigned short)n2 << 1;
    }
    twid_re = x_data[nd2];
    ytmp_data[n2].re = temp_re * twid_re + temp_im * 0.0;
    ytmp_data[n2].im = temp_re * 0.0 - temp_im * twid_re;
    if (n2 + 2 <= hnRows) {
      nd2 = n2 + 2;
                                     ...
                                     const emxArray_real_T *sintab)
{
  ...
  if (!tst) {
    if ((unsigned short)iy - 1 < 0) {
      istart = 0;
    } else {
      istart = (unsigned short)iy << 1;
    }
                                     ...
                                     emxArray_creal_T *y)
{
  ...
  if ((n2blue != 1) && (((unsigned int)nfft & 1U) == 0U)) {
    ...
    rt = (unsigned short)minNrowsNx;
    for (k = 0; k < rt; k++) {
      nInt2 = (nfft + k) - 1;
      re = x_data[k];
      y_data[k].re = wwc_data[nInt2].re * re;
      y_data[k].im = wwc_data[nInt2].im * -re;
    }
    minNrowsNx++;
    for (k = minNrowsNx; k <= nfft; k++) {
      y_data[k - 1].re = 0.0;
